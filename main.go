...
import (
	sap_api_caller "sap-api-integrations-defect-reads-rmq-kube/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-defect-reads-rmq-kube/SAP_API_Input_Reader"
	"sap-api-integrations-defect-reads-rmq-kube/config"
...
func callProcess(caller *sap_api_caller.SAPAPICaller, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	defect, defectText := extractData(msg.Data())
	accepter := getAccepter(msg.Data())
	caller.AsyncGetDefect(defect, defectText, accepter)
	return nil
}

func extractData(data map[string]interface{}) (defect, defectText string) {
	sdc := sap_api_input_reader.ConvertToSDC(data)
	defect = sdc.Defect.Defect
	defectText = sdc.Defect.DefectText
	return
}
